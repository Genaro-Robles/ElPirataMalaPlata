...
var _max_jumps = 3 # Máximo número de saltos
...
var _ora_sound = preload("res://assets/sounds/ora.mp3")
...
func _unhandled_input(event):
	...
	# Tecla A
	elif event.is_action_pressed("ora", true):
		character.velocity.x = 0
		_current_movement = _movements.ORA
	# Cuando se presiona la tecla b, lanzamos bomba
	elif event.is_action_released("ora"):
		character.velocity.x = 0
		attacking = false
...
func _on_animation_animation_finished():
	...
	if main_animation.get_animation() == 'dead_hit':
		...
		attacking = false
	elif main_animation.get_animation() == _movements.ORA:
		attacking = false
...
func _on_animation_frame_changed():	
	...
	elif main_animation.animation == "ora" and main_animation.frame == 1:
		_collision.set_deferred("disabled", false)	
	else:
